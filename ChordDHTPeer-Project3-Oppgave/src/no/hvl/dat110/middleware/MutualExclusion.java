...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		List<Message> msgPeers = removeDuplicatePeersBeforeVoting();

		// multicast the message to activenodes (hint: use multicastMessage)
		multicastMessage(message, msgPeers);

		// check that all replicas have replied (permission)
		boolean perm = areAllMessagesReturned(msgPeers.size());
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for (Message nodeMessage : activenodes) {
			...
			NodeInterface stub = Util.getProcessStub(nodeMessage.getNodeIP(), nodeMessage.getPort());

			// call onMutexRequestReceived()
			stub.onMutexRequestReceived(nodeMessage);
		}
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		case 2: {
			...
			int remoteClock = message.getClock();
